...
const QUOTE_EXPIRY_MS = 90_000;
export const MIN_SIGNABLE_TIME_TO_EXPIRY_MS = 3 * 60_000;
...
function buildAvailableQuote(
  ...
  return {
    ...
    quoteBudgetRaw: intent.amountType === "quote" ? toDusdcBaseUnits(Number(intent.amount)).toString() : null,
    quantityRaw: quote.quantityRaw.toString(),
    quantityDusdc,
    estimatedCostDusdc,
    estimatedCostRaw: quote.mintCostRaw.toString(),
    askPrice: quantityDusdc > 0 ? estimatedCostDusdc / quantityDusdc : null,
    bidPrice: quantityDusdc > 0 ? redeemPayoutDusdc / quantityDusdc : null,
    maxPayoutDusdc: quantityDusdc,
    maxPayoutRaw: quote.quantityRaw.toString(),
...
function unavailableQuote(
  ...
  return {
    ...
    quoteBudgetRaw: readyIntent?.amountType === "quote" ? toDusdcBaseUnits(Number(readyIntent.amount)).toString() : null,
    quantityRaw: null,
    quantityDusdc: null,
